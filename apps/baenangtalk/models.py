...
class Baenangtalk(models.Model):
    ...
    period = models.ForeignKey(BaenangtalkSubject, on_delete=models.CASCADE, verbose_name='여행시기')  # 여행시기 모델 외부키
    subject = models.ForeignKey(BaenangtalkPeriod, on_delete=models.CASCADE, verbose_name='주제')  # 주제 모델 외부키
      # 유저 모델 외부키
    ...
    # bae_like_by = models.ManyToManyField(User, related_name='liked_posts')  # 좋아요 누른 회원 (회원모델 가져와야함,,)
...
class BaenangtalkComment(models.Model):
    ...
    # User = models.ForeignKey(User, on_delete=models.CASCADE)  # 작성한 유저 (회원모델 가져와야함,,)
    bae_com_content = models.CharField('내용', max_length=100)  # 댓글내용
    bae_com_like = models.IntegerField('좋아요', default=0)  # 좋아요
    # bae_com_liked_by = models.ManyToManyField(User, related_name='liked_comments')  # 좋아요를 누른 회원 (회원모델 가져와야함,,)
